...
module.exports = {
    ...
    googleSheetInsert : async function(data){
        try {
            let endpoint = 'https://script.google.com/macros/s/AKfycbxHs0zhldJQoKEOXPwQ4TQarjCMRINMXvZ31tkQcOozace1CsODql7DAbQfR-C9mZorfw/exec';
            let formData = new FormData();

            // data가 객체라면 formData에 키-값 쌍 추가
            for (let key in data) {
                formData.append(key, data[key]);
            }

            let result = await axios.post(endpoint, formData);
    ...
    mailHook : function(gubun, dataObject, sendEmail, receiveEmail, subject, cc, attachments){
        ...
        let msg = makeForm.getForms(dataObject.type, dataObject);
