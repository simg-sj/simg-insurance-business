...
module.exports = {
    ...
    insuPdfSet: async function(data, reqNumber){
        ...
        // let ourputFileFullPath = basicPath+infoPath+'/'+cmpk+'/'+cmpk+'_joinInfo_'+saveDay+'.pdf';
        let ourputFileFullPath = basicPath+pathDay+"/"+reqNumber+"/"+pathDay+"_"+reqNumber+".pdf";
        console.log('파일 생성 경로 : ', ourputFileFullPath);
        /* 디렉토리 생성 */
        //  /uploads/[Info]  디렉토리가 없다면~ 생성
        if(!fs.existsSync(basicPath+pathDay)){
        ...
        // /uploads/valuemapInfo/[cmpk] 디렉토리가 없다면 ~ 생성
        if(!fs.existsSync(basicPath+pathDay + '/' + reqNumber)){
            fs.mkdirSync(basicPath+pathDay + '/' + reqNumber, {recursive:true}, (error)=>{
                if(error){
                    console.error('an error occurred : ', error);
                }else{
                    console.log('Directory is made : ', log);
        ...
        // 사고일시
        page1.drawText( data.accidentDate.substring(0,4)+'       '+data.accidentDate.substring(5,7)+'            '+data.accidentDate.substring(8,10), {
            x: accidentDateX,
            y: accidentDateY,
            font: customFont,
            size: 10,
